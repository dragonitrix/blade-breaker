	...
	.defines(function () {
		...
		MyGame = ig.Game.extend({
			...
			update: function () {
				...
				if (hitResult && (this.swords[0].isSlashing || this.swords[1].isSlashing)) {
					...
					if (this.swords[0].slashDir == this.swords[1].slashDir) {
						switch (this.swords[0].slashDir) {
							case this.slashDirs.UP:
								var angle1 = this.swords[0].angle;
								var angle2 = (this.swords[1].angle * -1) - 180;
								console.log(angle1+"  "+angle2);
								
								if (angle1 < angle2) {
									this.shout.break(4 - this.swords[1].health);
									if (4 - this.swords[1].health == 2) {
										this.swords[0].swing();
									}
									this.swords[1].break();
								} else {
									this.shout.break(4 - this.swords[0].health);
									if (4 - this.swords[1].health == 2) {
										this.swords[1].swing();
									}
									this.swords[0].break();
			...
			draw: function () {
				...
				var ctx = ig.system.context;
				ctx.font = "10pt arty-mouse";

				ctx.fillText(angle1.round(0)+" "+angle2.round(0),150,200);
